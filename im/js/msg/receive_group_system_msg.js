...
function onReadedSyncGroupNotify(notify) {
    ...
    var unreadNum = currentUnRead - seq;
    ...
    if(sess && sess.unread){
      sess.unread(unreadNum);
    }

    webim.Log.info("群消息同步的回调:已读消息情况 ## 未读数：GroupId:[" + notify.GroupId + "]:" + unreadNum, currentUnRead, seq);
    if (unreadNum > 0) {
        $(document.getElementById("badgeDiv_" + notify.GroupId)).text(unreadNum).show();
    } else {
