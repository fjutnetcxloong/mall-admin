...
function handleMsgSend(msgContent) {
  ...
  webim.sendMsg(msg, function (resp) {
    ...
    var showMsgP = document.getElementById('showMsg_' + selToID);
    showMsgP.innerHTML = msgContent;
  }, function (err) {
    console.log(msg, '尔特'); //alert(err.ErrorInfo);
    //提示重发
...
function showReSend(msg) {
  //resend a dom
  var resendBtn = $('<a href="javascript:;">重发</a>'); //绑定重发事件

  resendBtn.click(function () {
    //删除当前的dom
    $('#id_' + msg.random).remove(); //发消息处理
...
function onClose() {
  document.getElementById('webim_demo').style.display = 'none'; //离线

  webim.logout();
} //打开表情窗体


var showEmotionDialog = function showEmotionDialog() {
  if (emotionFlag) {
    $('#wl_faces_box').css({
      display: 'block'
    });
...
var turnoffFaces_box = function turnoffFaces_box() {
  $('#wl_faces_box').fadeOut('slow');
}; //选中表情


var selectEmotionImg = function selectEmotionImg(selImg) {
