...
var msgList = [];
var dateStart = null;
var dateEnd = null; //newMsgList 为新消息数组，结构为[Msg]

function onMsgNotify(newMsgList) {
  var sess, newMsg; //获取所有聊天会话
  ...
  for (var j in newMsgList) {
    ...
    if (!selToID) {
      //没有聊天对象, selToID 为全局变量，表示当前正在进行的聊天 id，当聊天类型为私聊时，该值为好友帐号，否则为群号。
      ...
      addSess(selType, selToID, newMsg.getSession().name(), headUrl, 0, 'sesslist', 'HEAD', null, convertMsgtoHtml(newMsg)); //新增一个对象
  ...
  for (var i in sessMap) {
    sess = sessMap[i]; // 更新demo 维护的recentSessMap的MsgGroupReadedSeq字段
...
function onBigGroupMsgNotify(newMsgList) {
  ...
  for (var i = newMsgList.length - 1; i >= 0; i--) {
    //遍历消息，按照时间从后往前
    newMsg = newMsgList[i];
    webim.Log.warn('receive a new group(AVChatRoom) msg: ' + newMsg.getFromAccountNick()); //显示收到的消息
...
function onMsgReadedNotify(notify) {
  ...
  if (sessMap) {
    ...
    unread = unread > 0 ? unread : 0; //更新sess的未读数

    sessMap.unread(unread); // console.debug('更新C2C未读数:',notify.From_Account,unread);
    //更新页面的未读数角标
