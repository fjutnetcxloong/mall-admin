...
function getHistoryMsgCallback(msgList, prepage) {
  var msg;
  prepage = prepage || false; //如果是加载前几页的消息，消息体需要prepend，所以先倒排一下
  ...
  for (var j in msgList) {
    //遍历新消息
    msg = msgList[j];
    console.log(msg.getSession().id() ,selToID,'开了个人头')
    if (msg.getSession().id() == selToID) {
      //为当前聊天对象的消息
      selSess = msg.getSession(); //在聊天窗体中新增一条消息
...
var getLastC2CHistoryMsgs = function getLastC2CHistoryMsgs(cbOk, cbError) {
...
var getPrePageC2CHistoryMsgs = function getPrePageC2CHistoryMsgs(cbOk, cbError) {
  if (selType == webim.SESSION_TYPE.GROUP) {
    alert('当前的聊天类型为群聊天，不能进行拉取好友历史消息操作');
    return;
  }
...
var getLastGroupHistoryMsgs = function getLastGroupHistoryMsgs(cbOk) {
...
var getPrePageGroupHistoryMsgs = function getPrePageGroupHistoryMsgs(cbOk) {
  if (selType == webim.SESSION_TYPE.C2C) {
    alert('当前的聊天类型为好友聊天，不能进行拉取群历史消息操作');
    return;
  }
