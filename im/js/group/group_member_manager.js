...
function ggmOperateFormatter(value, row, index) {
  // return ['<a class="pencil ml10" href="javascript:void(0)" title="修改群成员角色">', '<i class="glyphicon glyphicon-pencil"></i>', '</a>', '<a class="time ml10" href="javascript:void(0)" title="设置禁言时间">', '<i class="glyphicon glyphicon-time"></i>', '</a>', '<a class="remove ml10" href="javascript:void(0)" title="移除该成员">', '<i class="glyphicon glyphicon-remove"></i>', '</a>'].join('');
    return ['<a class="remove ml10" href="javascript:void(0)" title="移除该成员">', '<i class="glyphicon glyphicon-remove"></i>', '</a>'].join('');
} //定义群组成员表格每行的按钮单击事件
...
window.ggmOperateEvents = {
    ...
    switch ($('#ggm_my_role').val()) {
      ...
      case '群主':
        ...
        if (row.Role == '群主') {
          ...
          $('#mgm_account').val(row.Member_Account); //设置群身份

          var role_en = webim.Tool.groupRoleCh2En(row.Role); //$("input[type=radio][name='mgm_role_radio'][value=" + role_en + "]").attr("checked", 'checked');
  ...
  'click .time': function clickTime(e, value, row, index) {
    switch ($('#ggm_my_role').val()) {
      case '成员':
        alert('你不是管理员，无法进行此操作');
        break;
...
var getGroupMemberInfo = function getGroupMemberInfo(group_id) {
...
var modifyGroupMemberRole = function modifyGroupMemberRole() {
...
var forbidSendMsg = function forbidSendMsg() {
...
var modifyGroupMsgFlag = function modifyGroupMsgFlag() {
...
var deleteGroupMember = function deleteGroupMember() {
  if (!confirm('确定移除该成员吗？')) {
    return;
  }
...
var addGroupMember = function addGroupMember() {
  var options = {
    ...
    }] // userIDList: [$('#agm_account').val()]

  };
  console.log(options, '圣诞节快乐风了');
