...
function changeAppType(item) {
  ...
  if (appType == 1) {
    ...
    $('#sdkAppIdDiv').show();
    $('#accountTypeDiv').show(); // $('#accountModeDiv').show();
  }
}
...
function showLoginDialog() {
  $('#select_app_dialog').modal('hide'); //显示登录窗体
...
function independentModeLogin() {
  ...
  loginInfo.sdkAppID = '1400243962';//1400280073 正式
  loginInfo.userSig = '';
  webimLogin(function () {
    $('#login_dialog').modal('hide');
  }, function () {// $('#login_dialog').modal('show');
  });
} //初始化demo
...
function initDemoApp() {
  ...
  $('#t_my_menu').menu();
  $('#send_msg_text').focus(); //初始化我的加群申请表格

  initGetApplyJoinGroupPendency([]); //初始化我的群组系统消息表格

  initGetMyGroupSystemMsgs([]); //初始化我的好友系统消息表格

  initGetMyFriendSystemMsgs([]); // new webim.Session(webim.SESSION_TYPE.C2C, '10632');
  //初始化我的资料系统消息表格

  initGetMyProfileSystemMsgs([]); //初始化好友和群信息
...
function initInfoMap(cbOk) {
  //读取我的好友列表
  initInfoMapByMyFriends( //读取我的群组列表
