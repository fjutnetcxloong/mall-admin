...
function gmfOperateFormatter(value, row, index) {
    return ['<a class="envelope" href="javascript:void(0)" title="发消息">', '<img src="./img/IM-zhu.png" alt=""/>', '</a>',
        /*'<a class="ban-circle" href="javascript:void(0)" title="拉黑">',
  '<i class="glyphicon glyphicon-ban-circle"></i>',
  '</a>',*/
        '<a class="remove ml10" href="javascript:void(0)" title="删除">', '<img src="./img/icon_deleat.png"alt=""/>', '</a>'].join('');
} //我的好友表格每行的操作按钮点击事件
...
window.gmfOperateEvents = {
    ...
    'click .ban-circle': function clickBanCircle(e, value, row, index) {
...
const applyAddFriend = function applyAddFriend() {
    const len = webim.Tool.getStrBytes($('#af_add_wording').val());
    ...
    webim.applyAddFriend(options, function(resp){
        ...
        if (err.ResultItem[0].ResultCode === 30001) {
            alert('对方已经是您的好友');
        } else {
            alert(err.ErrorInfo);
        }
...
const deleteFriend = function deleteFriend() {
    if (!confirm('确定删除该好友吗？')) {
        return;
    }

    let to_account = [];
    to_account = [$('#df_to_account').val()];
...
const getMyFriend = function getMyFriend() {
    //清空
    ...
    const options = {
        ...
        TagList: ['Tag_Profile_IM_Nick', 'Tag_SNS_IM_Remark', 'Tag_Profile_IM_Gender']
    };
    webim.getAllFriend(options, function(resp){
        if (resp.FriendNum > 0) {
            const table_friends_data = [];
            const friends = resp.InfoItem;
...
function sendC2cMsg() {
    ...
    msgovermain.style.display = 'none';
    ...
    webim.sendMsg(msg, function(resp){
        ...
        setSelSessStyleOff(1);//清除平台消息选中
        setSelSessStyleOff(2);//清除订单消息选中
...
const initInfoMapByMyFriends = function initInfoMapByMyFriends(cbOK) {
    const options = {
        From_Account: loginInfo.identifier,
        TimeStamp: 0,
        StartIndex: 0,
    ...
    webim.getAllFriend(options, function(resp){
        if (resp.FriendNum > 0) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                const friend_account = friend.Info_Account;
                let friend_name = friend_image = '';
                ...
                const key = webim.SESSION_TYPE.C2C + '_' + friend_account;
                infoMap[key] = {
                    name: friend_name,
                    image: friend_image
                };
