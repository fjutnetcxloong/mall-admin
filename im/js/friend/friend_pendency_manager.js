...
function gpOperateFormatter(value, row, index) {
  return ['<a class="edit" href="javascript:void(0)" title="处理">', '<i class="glyphicon glyphicon-edit"></i>', '</a>', '<a class="remove ml10" href="javascript:void(0)" title="删除">', '<i class="glyphicon glyphicon-remove"></i>', '</a>'].join('');
} //我的好友申请表格每行的按钮点击事件
...
var getPendency = function getPendency(notifyFlag) {
...
var deletePendency = function deletePendency(del_account) {
  var options = {
    From_Account: loginInfo.identifier,
    PendencyType: 'Pendency_Type_ComeIn',
    To_Account: [del_account]
  };
  webim.deletePendency(options, function (resp) {
    //在表格中删除对应的行
...
var responseFriend = function responseFriend() {
  ...
  webim.responseFriend(options, function (resp) {
    //在表格中删除对应的行
    $('#get_pendency_table').bootstrapTable('remove', {
      field: 'To_Account',
    ...
    if (response_friend_item[0].ResponseAction == 'Response_Action_AgreeAndAdd') {//getAllFriend(getAllFriendsCallbackOK);
    }
