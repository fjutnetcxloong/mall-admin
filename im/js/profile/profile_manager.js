  ...
  webim.getProfilePortrait(options, function (resp) {
    if (resp.UserProfileItem && resp.UserProfileItem.length > 0) {
      for (var i in resp.UserProfileItem) {
        ...
        $('#spp_nick').val(nick); //$("input[type=radio][name='spp_gender_radio'][value=" + gender + "]").attr("checked", "checked");//此方法，在IE浏览器下，第一次查看个人资料时，选中没有效果
...
function spOperateFormatter(value, row, index) {
  return ['<a class="plus" href="javascript:void(0)" title="添加好友">', '<i class="glyphicon glyphicon-plus"></i>', '</a>'].join('');
} //搜索用户表格每行按钮单击事件


window.spOperateEvents = {
  'click .plus': function clickPlus(e, value, row, index) {
    $('#af_to_account').val(row.To_Account); //重新获取对方的加好友方式
...
var searchProfileByUserId = function searchProfileByUserId() {
  ...
  webim.getProfilePortrait(options, function (resp) {
    ...
    if (err.UserProfileItem[0].ResultCode === 40003) {
      alert('请输入正确的账号');
    } else {
      alert(err.ErrorInfo);
    }
...
var searchProfileAllowTypeByUserId = function searchProfileAllowTypeByUserId(to_account, cbok, cberr) {
  var allowType = '需要确认';
  ...
  var tag_list = ['Tag_Profile_IM_AllowType'];
  var options = {
...
var setProfilePortrait = function setProfilePortrait() {
  ...
  console.log(profile_item,'会计师的反馈');
  if (image) {
    //如果设置了头像URL
    profile_item.push({
  ...
  console.log(options,'水电费考虑考虑')
  webim.setProfilePortrait(options, function (resp) {
