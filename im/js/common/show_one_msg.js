...
function addMsg(msg, prepend) {
  ...
  if (msg.isSend) {
    msghead.className = 'msghead-my';
    msgbody.className = 'msgbody-my';
  } else {
  ...
  if (msg.isSend) {
    msghead.innerHTML = '<p><span>' + webim.Tool.formatTimeStamp(msg.getTime()) + '</span>' + webim.Tool.formatText2Html(fromAccountNick) + '</p>' + "<img class='headurlClass' src='" + fromAccountImage + "'>" + '&nbsp;&nbsp;';
  } else {
    msghead.innerHTML = "<img class='headurlClass' src='" + fromAccountImage + "'>" + '<p>' + webim.Tool.formatText2Html(fromAccountNick) + '<span>' + webim.Tool.formatTimeStamp(msg.getTime()) + '</span></p>';
  } //解析消息
  //获取消息子类型
  ...
  switch (subType) {
    ...
    case webim.GROUP_MSG_SUB_TYPE.LOVEMSG:
      ...
      msgPre.innerHTML = '[群点赞消息]' + convertMsgtoHtml(msg); //展示点赞动画
  ...
  onemsg.appendChild(msgbody); //消息列表
  ...
  if (prepend) {
    ...
    msgflow.appendChild(onemsg); //300ms后,等待图片加载完，滚动条自动滚动到底部
  ...
  for (var i = 0; i < count; i++) {
    ...
    switch (type) {
      case webim.MSG_ELEMENT_TYPE.TEXT:
        var eleHtml = convertTextMsgToHtml(content); //转义，防XSS
      ...
      case webim.MSG_ELEMENT_TYPE.CUSTOM:
        var eleHtml = convertCustomMsgToHtml(content); //转义，防XSS

        html += webim.Tool.formatText2Html(eleHtml);
        break;

      case webim.MSG_ELEMENT_TYPE.GROUP_TIP:
        var eleHtml = convertGroupTipMsgToHtml(content); //转义，防XSS
...
function convertGroupTipMsgToHtml(content) {
  ...
  switch (opType) {
    case webim.GROUP_TIP_TYPE.JOIN:
      //加入群
      userIdList = content.getUserIdList(); //text += opUserId + "邀请了";
