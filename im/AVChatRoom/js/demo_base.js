...
function showMsg(msg) {
    ...
    const maxDisplayMsgCount = 4;
    //var opacityStep=(1.0/4).toFixed(2);
    const opacityStep = 0.2;
    let opacity;
    const childrenLiList = $('#video_sms_list').children();
    ...
    const colorList = ['red', 'green', 'blue', 'org'];
    const index = Math.round(fromAccount.length % colorList.length);
...
function tlsGetUserSig(res) {
    ...
    if (res.ErrorCode == webim.TLS_ERROR_CODE.OK) {
        ...
        const accountType = webim.Tool.getCookie('accountType');
        if (accountType) {
...
function imageClick(imgObj) {
    const imgUrls = imgObj.src;
    const imgUrlArr = imgUrls.split('#'); //字符分割
    const smallImgUrl = imgUrlArr[0]; //小图
    const bigImgUrl = imgUrlArr[1]; //大图
    const oriImgUrl = imgUrlArr[2]; //原图
...
function onSendMsg() {
    ...
    //获取消息内容
    let msgtosend = $('#send_msg_text').val();
...
function sendGroupLoveMsg() {
    ...
    //webim.GROUP_MSG_SUB_TYPE.TIP-提示消息(不支持发送，用于区分群消息子类型)，
    //webim.GROUP_MSG_SUB_TYPE.REDPACKET-红包消息，优先级最高
    const subType = webim.GROUP_MSG_SUB_TYPE.LOVEMSG;

    const msg = new webim.Msg(selSess, isSend, seq, random, msgTime, loginInfo.identifier, subType, loginInfo.identifierNick);
    const msgtosend = 'love_msg';
...
function showLoveMsgAnimation() {
    //点赞数加1
    const loveCount = $('#user-icon-like').html();
    $('#user-icon-like').html(parseInt(loveCount) + 1);
    const toolDiv = document.getElementById('video-discuss-tool');
    const loveSpan = document.createElement('span');
    const colorList = ['red', 'green', 'blue'];
    const max = colorList.length - 1;
    const min = 0;
    const index = parseInt(Math.random() * (max - min + 1) + min, max + 1);
    const color = colorList[index];
