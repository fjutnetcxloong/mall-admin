...
module.exports = (env = {}) => {
    ...
    let baseConfig = {
        ...
        plugins: [
            ...
            new copyWebpackPlugin([{
                ...
                to: path.resolve(__dirname, OUTPUTDIR + ENV.path() + '/public') //打包到dist下面的public
            }]),
            new webpack.DefinePlugin({
                /* 定义环境变量 NODE_ENV ，在开发代码中区分环境 */
                'process.env': {
                    NODE_ENV: JSON.stringify(ENV.path()),
                    NATIVE_HEADER: JSON.stringify(false),
                }
            }),
            ...(ENV.isDevEnv()
                ? [
    ...
    let clientConfig = _.merge(baseConfig, {
        name: 'client',
        target: 'web',
        ...
        output: {
            path: path.resolve(__dirname, OUTPUTDIR + ENV.path()),
