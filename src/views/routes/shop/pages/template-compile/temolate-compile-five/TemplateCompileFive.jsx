...
import './TemplateCompileFive.less';
import ModelFive from  '../model/ModelFive';
...
class TemplateCompileFive extends BaseComponent {
    state = {
        popping: '', //选择那个选择框
    ...
    getDefaultModel = () => {
        this.fetch(api.modelMetail, {data: {mol_id: 5}}).subscribe(res => {
            if (res && res.status === 0) {
    ...
    modelSave = () => {
        ...
        if (picurl && picurl.size >= 9 && content.get('banner').size > 0) {
            //这里是判断所有的字段都是否有填写
    ...
    saveEnd = (content, picurl, draf) => {
        this.setState({//弹框提示保存中
            ...
            const bannerImg = picurl.slice(0, 9).concat(content.get('banner').map(item => item.get('url')));
            ...
            this.fetch(api.modelSave, {data: {type: draf ? 0 : 1, mol_id: 5, content: JSON.stringify(saveObj)}})
                .subscribe(res => {
                    if (res.status === 0) {
                        const pasArr = [];
    ...
    render() {
        ...
        return (
            <React.Fragment>
                <Row className="template-compile-five back">
                    <Col span={8}>
                        <div className="static">
                            ...
                            <img src={require('../../../../../../assets/images/summer-five.png')} alt=""/>
                        </div>
                    </Col>
                    <Col span={8}>
                        <ModelFive
...
export default TemplateCompileFive;
//
