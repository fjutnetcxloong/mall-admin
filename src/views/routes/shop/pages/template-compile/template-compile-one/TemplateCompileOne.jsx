...
import './TemplateCompileOne.less';
import '../../../index.less';
...
// TODO: defaultMOdeInfo 应考虑用 immutable数据
class TemplateCompileOne extends BaseComponent {
    ...
    //获取，模板信息
    getDefaultModel = () => {
        this.fetch(api.modelMetail, {data: {mol_id: 1}}).subscribe(res => {
            if (res && res.status === 0) {
    ...
    saveEnd = (content, picurl, draf) => {
        this.setState({//弹框提示保存中
            ...
            this.fetch(api.modelSave, {data: {type: draf ? 0 : 1, mol_id: 1, content: JSON.stringify(saveObj)}})
                .subscribe(res => {
                    if (res.status === 0) {
                        const pasArr = [];
    ...
    render() {
        ...
        return (
            <React.Fragment>
                <Row className="template-compile-one back">
                    <Col span={8}>
                        <div className="static">
                            ...
                            <img src={require('../../../../../../assets/images/summer.png')} alt=""/>
                        </div>
                    </Col>
                    <Col span={8}>
                        <ModelOne
                    ...
                    <Col span={8} className="template-compile-one-right">
                ...
                <div className="edit-template-button">
                    ...
                    <Button onClick={this.modelSaveDraft}>保存草稿</Button>
